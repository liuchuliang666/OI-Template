> 本文转载（或修改）自 [OI-Wiki](https://oi-wiki.org/graph/shortest-path/)

## 定义

（还记得这些定义吗？在阅读下列内容之前，请务必了解 [图论相关概念](./一些概念.md) 中的基础部分。）
...
??? code "实现"
    ```cpp
    --8<-- "docs/Graph/code/Bellman–Ford.cpp:16:40"
    ```
...
??? code "实现"
    ```cpp
    --8<-- "docs/Graph/code/SPFA.cpp:15:33"
    ```
...
这里同时给出 $O(m \log m)$ 的优先队列做法实现。

??? code "实现"
    ```cpp
    --8<-- "docs/Graph/code/dijkstra.cpp:15:32"
    ```
...
我们给出 [P5905 【模板】全源最短路（Johnson）](https://www.luogu.com.cn/problem/P5905) 的代码。

??? code "实现"
    ```cpp
    --8<-- "docs/Graph/code/johnson.cpp"
    ```
...
-   允许至多 $k$ 次改变路径成本等操作的最短路问题：[分层图最短路](https://www.luogu.com.cn/problem/P13271)。
