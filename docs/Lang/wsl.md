> 本文转载（或修改）自 [OI-Wiki](https://oi-wiki.org/tools/wsl)
...
| 集成开发环境（IDE） | CodD::Blocks              | `20.03`            | C/C++ IDE                                                           |
...
考虑先导出 WSL 内核包，再重新加载内核包实现。

```powershell
wsl --export Ubuntu-22.04 D:\wsl-export.tar # 导出内核包为 D:\wsl-export.tar，这里 wsl 版本为 Ubuntu-22.04
wsl --unregister Ubuntu-22.04 # 卸载原有的 wsl
wsl --import Ubuntu-22.04 D:\wsl D:\wsl-export.tar # 将内核包加载至 D:\wsl 目录下
```
...
等待一两分钟时间，系统会提示创建新的用户帐户。
    ...
    Enter new UNIX usernamD: chtholly
...
如果习惯在 Windows 环境下使用 [Visual Studio Code](https://code.visualstudio.com/) 进行代码编辑，可以安装 VS Code 中的 `Remote - WSL` 插件，更方便地对 WSL 系统中的文件进行编辑。
...
1.  启用「虚拟机平台」功能
    ...
    ```shell
    dism.exe /online /enable-feature /featurenamD:VirtualMachinePlatform /all /norestart
    ```
