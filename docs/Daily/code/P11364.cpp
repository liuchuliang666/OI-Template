...
#define rs ((x) << 1 | 1)
    ...
    int query(int x, int l, int r, int pl, int pr)
    {
        ...
        int res = -1e9;
        if (pl <= mid) chkmx(res, query(ls, l, mid, pl, pr));
        if (mid < pr) chkmx(res, query(rs, mid + 1, r, pl, pr));
        return res;
    }
} sgt;
int main()
{
    ...
    dfs(1, 0);
    for (int i = 1; i < n; i++) d[i] = deep[getlca(i, i + 1)];
    for (int i = 1; i < n; i++)
    {
    ...
    for (int i = 1; i < n; i++) lines[i] = {lsh[i], rsh[i] + 1, d[i]};
    cin >> qq;
    for (int i = 1; i <= qq; i++) cin >> q[i].l >> q[i].r >> q[i].k, q[i].pos = i;
    sgt.clear();
    ...
    {
        while (idx < n - 1 && lines[idx + 1].y >= q[i].r) idx++, sgt.update(1, 1, n, lines[idx].x, lines[idx].v);
    ...
    {
        while (idx < n - 1 && lines[idx + 1].y - lines[idx + 1].x + 1 >= q[i].k) idx++, sgt.update(1, 1, n, lines[idx].y, lines[idx].v);
    ...
    sort(q + 1, q + 1 + qq, [](node a, node b)
         { return a.k < b.k; });
    for (int i = 1; q[i].k == 1; i++) chkmx(ans[q[i].pos], sgt.query(1, 1, n, q[i].l, q[i].r));
